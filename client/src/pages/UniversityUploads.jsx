function UniversityUploads() {
    ...
    <div className="w-full h-full overflow-y-auto">
        <h1 className="font-semibold text-3xl w-full text-center m-2">My Uploads</h1>
      ...
      {projectFeed.map((item, index) => {
        return (
          <div key={index} className="w-1/2 rounded-md shadow-md border p-2">
            ...
            <div className="p-2 w-full overflow-x-auto whitespace-normal break-normal">{/* Added whitespace-normal class here */}
              <p>{item.description}</p>
            </div>
            <span>Owner-</span>
            <span>{item.owner.name}</span>
            <img className="inline-block ps-4" src={item.owner.profilePhoto} alt="pfp not available" />
            <button className="active:scale-95 mb-1 w-full px-4 py-2 font-semibold text-sm bg-primary hover:bg-primary-light text-white rounded-md shadow-sm">View Details</button>
          </div>
        );
      })}
    </div>
...
export default UniversityUploads;